...
Designed a high-speed, low-power pipelined 32-bit adder operating in the sub-threshold region for ultra-low power digital VLSI applications. The design utilizes static CMOS logic and a Kogge-Stone prefix-tree architecture, optimized to run at a target operating frequency of 100 MHz.

### 1. Sub-threshold Logic Design & Physics Challenges

Operating a circuit in the sub-threshold region (Vdd < Vth) introduces severe physical design constraints:
* **Exponential Sensitivity:** The drive current (Isub) depends exponentially on the threshold voltage (Vth) and temperature, rendering the design highly sensitive to process, voltage, and temperature (PVT) variations and severely degrading noise margins.
...
* **Pipeline Partitioning:** Pipelined the prefix network using custom **low-voltage transmission-gate registers** to satisfy the 100 MHz clock frequency constraint. The low logic depth per pipeline stage allowed aggressive voltage scaling while maintaining structural timing margins.